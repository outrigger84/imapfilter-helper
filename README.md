...
| `run-all` | Convenience command that runs `build-cache`, `evaluate`, and `execute` sequentially. | `--dry-run` – perform a full simulation without IMAP writes (and without opening an IMAP connection).<br>`--all-folders` – include every folder when building the cache.<br>`--folder NAME` – restrict all three phases to the specified folder(s).<br>`--limit` / `--order` – share the cache-limiting flags from `build-cache`.<br>`--backup` – export cached messages as mbox files during the cache phase.<br>`--debug-headers` – log message headers while evaluating rules.<br>`--strict` / `--verify-moves` – mirror the execute integrity checks when applying actions.<br>`--verbose` – surface detailed progress and IMAP replies during the evaluate/execute phases.<br>`--action-limit` – cap how many pending actions are executed during the final phase. |
